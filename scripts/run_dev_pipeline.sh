...
#   0b  Join Cap3D captions + download meshes → manifest
#   1   Teacher LLM code gen → Blender validation via Modal → synthetic SFT pairs
#
...
# ── Phase 0b: Caption join + mesh download ───────────────────────────
banner "Phase 0b: Building manifest (Cap3D captions + mesh download)"
...
banner "Phase 1: Synthetic generation (teacher LLM → Blender → validation)"
